...
trait Stream[+A] {
  ...
  def map[B](f: A => B): Stream[B] = {
    this.foldRight(Stream.empty[B]) { (elem, acc) =>
      Cons(() => f(elem), () => acc)
    }
  }
  ...
  def startsWith[B](s: Stream[B]): Boolean = {
    // s.foldRight((true, this)) {
    //     case (elem, (equal, Cons(head, tail))) =>
    //       println(s"Equal: $equal - Head: ${head()} - Elem: $elem")
    //       (equal && head() == elem, tail())
    //     case (_, (equal, Empty)) =>
    //       println("empty case")
    //       (false, Empty)
    //   }
    //   ._1
    zipAll(s).takeWhile { case (elem1, elem2) => elem2.isDefined }.forAll {
      case (elem1, elem2) => elem1 == elem2
    }
  }

  // zipped (1, 1), (2, 2), (3, 3), (4, None)
...
object Stream {
  ...
  def constant[A](a: A): Stream[A] =
    Stream.cons(a, constant(a)) //second parameter of cons is lazy -> it will only compute next value if needed
  ...
  def unfold[A, S](z: S)(f: S => Option[(A, S)]): Stream[A] = f(z) match {
    case Some((nextValue, nextState)) =>
      cons(nextValue, unfold(nextState)(f)) //you need to create a stream, so that's why you call cons
    case None => Empty
  }

  def from2(n: Int): Stream[Int] = unfold(n)(z => Some(z, z + 1))
  val fibs2: Stream[Int] = unfold((0, 1)) {
    case (z, y) => Some((z + y, (y, z + y)))
  }
...
object StreamMain {
  def main(args: Array[String]): Unit = {
    ...
    println(s2.startsWith(s1))

    // println(Stream.fibs.startsWith(Stream(0, 1, 1)))
  }
}
